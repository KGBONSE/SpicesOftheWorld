# Episode 5 — The Maghreb: Cumin, Cinnamon & Harissa

First episode with a genuine ancient-history spice (cumin, 5,000+ years)
and a blend with real measurements straight from the source. Grounded in
`knowledge-base/regional/africa-spice-notes.md` (Maghreb section) and the
Cumin and Cinnamon profiles in `knowledge-base/spice-profiles/`.

## Intro (~10 sec)

"Today we're heading to the Maghreb — Morocco, Algeria, Tunisia, Libya —
the place the book itself calls the portal where Africa meets the Middle
East and Europe."

## 1. Personal Hook

`[DRAFT — swap in your own real detail before locking]` I remember the
first time I actually made harissa myself, here on the farm — I opened
the jar afterward and the smell hit me somewhere between Ghana and
somewhere I'd never actually been. Cumin, chilli, all these warm spices I
already knew, but arranged completely differently. That's the moment it
landed for me that "Spices of Africa" couldn't just mean Ghana — Morocco,
Algeria, this whole other flavour world, it's still Africa, and I wanted
this brand to actually say that.

**[ON-SCREEN TEXT]:** "Spices of Africa — not just Ghana"
**[B-ROLL]:** The actual harissa jar if you still have it, market/spice
stall footage if available, or map graphic

## 2. Geography & Origin

Morocco, Algeria, Tunisia, Libya — the Maghreb. The book calls it the
portal where Africa meets the Middle East and Europe, and you can taste
that in the food — sweet and mild overall, cumin doing the everyday
heavy lifting, cinnamon bringing warmth to fruit-laden tagines.

**[ON-SCREEN TEXT]:** "The Maghreb — where Africa meets the Middle East and Europe"
**[B-ROLL]:** Map graphic

## 3. Trade & Migration History

Cumin might be the oldest spice we've covered — there's evidence of it in
Egyptian pyramids, more than 5,000 years old. The Romans loved it so much
Pliny the Elder called it the "king of condiments." From the 7th century,
Arab traders carried it along the caravan routes into North Africa and
east as far as Iran, India, Indonesia, China — it's in baharat, garam
masala, panch phoran, ras el hanout, all the same seed. And here's the
twist for this region specifically — for centuries the Maghreb wasn't
just receiving influence, Berbers themselves ruled much of Spain as
Al-Andalus. This place has always moved in more than one direction.

**[ON-SCREEN TEXT]:** "Cumin — 5,000+ years, Egyptian pyramids to Pliny the Elder"
**[B-ROLL]:** Historical map/timeline animation

## 4. The Science

Cumin's flavour comes from cuminaldehyde — musky, spicy, and it's rarely
found anywhere else, except, interestingly, in cinnamon too. That's part
of why the two work so well together here — cinnamon's own compound,
cinnamaldehyde, adds an earthier warming depth right alongside it. Toast
the cumin seeds first, bruise them, dry-fry them, and you get roasted
notes you just don't get from the raw seed.

Both of these carry real research weight too — cumin's been linked to
genuine digestive and metabolic benefits, and even a small daily amount of
true cinnamon shows up in studies improving blood sugar and cholesterol.
Caraway, the third seed here, has its own research on digestion and blood
sugar, and the chilli the whole paste is built on links in large
population studies to lower cardiovascular risk. Harissa reads as pure
indulgence — the spice list says otherwise.

**[ON-SCREEN TEXT]:** "Cumin + cinnamon share real chemistry — toast the seeds first" / "Cumin, cinnamon, caraway — digestion, blood sugar, cholesterol" / "Chilli: linked to lower cardiovascular risk"
**[B-ROLL]:** Seeds toasting, close-up

## 5. The Blend — Harissa

Soak 100 grams of dried red chillies — Baklouti if you can get them — in
hot water for 30 minutes. Then blend with 3 to 4 garlic cloves, salt,
lemon juice, ground cumin, ground coriander, ground caraway, and olive
oil, into a paste.

**[ON-SCREEN TEXT]:** "HARISSA: 100g dried red chillies + 3–4 garlic cloves + salt + lemon juice + cumin + coriander + caraway + olive oil"
**[B-ROLL]:** Chillies soaking, blending into paste

*(Note: chilli and garlic quantities are exact from the source — the
ground spices are "to taste," not measured. Worth test-cooking to lock in
your own ratio there.)*

## 6. The Dish

Harissa turns up all over Maghrebi cooking — stirred into couscous broth,
spooned into shakshuka's tomato-and-pepper sauce, rubbed onto merguez
before it hits the grill. But the pairing that shows off cumin and
cinnamon best is the one we're doing today.

**[ON-SCREEN TEXT]:** "Harissa also lives in: couscous · shakshuka · merguez"

Stir that harissa through a tagine already built on cumin and cinnamon —
fruit, meat, that slow-cooked warmth — and you've got the Maghreb on a
plate. Harissa doesn't need its own dish, really — it's the thing that
finishes everything else.

**[ON-SCREEN TEXT]:** "TAGINE + HARISSA"
**[B-ROLL]:** Tagine simmering, harissa being stirred in

## Africa Link / Throughline Close

That jar of harissa is long finished, but I still remember that smell.
What I like about the Maghreb is it breaks the story a little — paprika
and allspice turning up here came from the Americas, not the other way
round, and Berbers ruling part of Spain means this corner of Africa was
exporting its own influence outward too, not just receiving everyone
else's. That's the same thing I'm trying to do from London — not just
carrying Africa's spices somewhere new, but making sure the story travels
with them, the way it always has.

**[ON-SCREEN TEXT]:** "Africa didn't just receive the world's spice trade — it shaped it"
**[B-ROLL]:** Sidcup farm footage, product shot

## Closing CTA

"Full written recipe and the exact blend I used are up at fudipeople.com."

**[ON-SCREEN TEXT]:** "Recipe + get the blend — fudipeople.com"

---

## Status

Full draft, ready to read/shoot except the hook. Every fact traces to
`knowledge-base/regional/africa-spice-notes.md` (Maghreb section) and the
Cumin/Cinnamon profiles in `knowledge-base/spice-profiles/`.

Open items: lock in your own real detail for the Personal Hook draft below
(marked `[DRAFT]`) — the harissa-jar moment is invented as a placeholder,
not from Kofi — and test-cooking the ground-spice ratios in harissa
(chilli and garlic quantities are exact from the source; the rest is "to
taste").

**2026-07-25:** Added a quick "other uses" mention (couscous, shakshuka,
merguez) at the top of The Dish section, per the new episode framework
beat 5 pattern. General culinary knowledge, not from the book.

**2026-07-27:** Added a Health Benefits line to The Science section
(digestive/metabolic for cumin, blood sugar/cholesterol for true
cinnamon), sourced from both spices' own profiles in
`knowledge-base/spice-profiles/by-flavour-compound/02_warming_terpenes/cumin.md`
and `knowledge-base/spice-profiles/core-profiles-master.md` — external
research, not from the book. Note: the cinnamon profile's benefits are
for true (Ceylon) cinnamon specifically, lower-coumarin than cassia — this
episode doesn't specify which, worth checking which one Kofi actually uses
in the tagine before filming.

**2026-07-28:** Drafted the Personal Hook itself (making harissa on the
farm, the smell bridging Ghana and the Maghreb) and echoed the same jar
image at the top of the Africa Link close. Marked `[DRAFT]`, not `[NEEDS
KOFI]` — every specific detail is invented as a starting point, not a real
Kofi memory, so swap in your own before this locks.
